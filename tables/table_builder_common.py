...
# ---------------------------------------------------------------------------
# Report-level data-selection predicates — MOVED to document_model/filters.py.
#
# These decide WHICH data appears in a report (sexes/assays/organs/genes/gene
# sets); they are report-level data selection, not table-cell construction, and
# are used across pipeline/ narrative/ rendering/ as well as here.  They were
# historically extracted into this "table builder utility" module by proximity
# (the organ matcher started inside body_weight_table.py).  They now live in
# document_model/filters.py beside the filter *config* loaders.
#
# Re-exported here so existing `from tables.table_builder_common import
# organ_allowed` (etc.) call sites keep working.  Prefer importing from
# document_model.filters in new code.
# ---------------------------------------------------------------------------
from document_model.filters import (  # noqa: E402,F401
    filter_allows,
    organ_allowed,
    sex_allowed,
    assay_allowed,
