...
# ---------------------------------------------------------------------------
# Per-node-type emitters (the BITS/JATS `_DISPATCH` table)
# ---------------------------------------------------------------------------
# Until 2026-09-18 this surface dispatched through an if/elif chain whose final
# `else` silently skipped any node type it did not name — eight registered types
# (cover, title-page, toc, tables-list, appendix, freeform-page, freeform-block,
# page-break) fell through without a trace, and nothing checked the chain
# against RENDERABLE_NODE_TYPES the way the HTML / LaTeX / Word emitters are
# checked at import.  The table below makes every decision explicit: each
# registered type maps to a handler, the "handled elsewhere in the book shell"
# types map to a documented no-op, and the true projection gaps map to a
# handler that leaves a visible TODO comment in the XML.  assert_dispatch_covers
# then fails the import if the registry and this table ever drift.
#
# Handler contract: (parent, node, data) -> None; append the node's blocks to
# `parent` (a <body> or <sec>).  Recursion into children happens INSIDE the
# handler where the content model requires it (heading-only, narrative+tables).

def _emit_narrative(parent: etree._Element, node: DocNode, data: dict) -> None:
    """narrative / narrative+tables → one <sec> (prose, then child tables)."""
    parent.append(_narrative_sec(node, data))


def _emit_heading_only(parent: etree._Element, node: DocNode, data: dict) -> None:
    """heading-only → a titled <sec> that nests its children."""
    sec = E.sec({"id": f"sec-{node.id}"})
    if node.title:
        sec.append(E.title(node.title))
    for child in node.children:
...
def _emit_raw_table_blocks(parent: etree._Element, node: DocNode, data: dict) -> None:
    """table / incidence-table → raw <table-wrap> block(s) inside the parent
    narrative <sec> (they sit among blocks, never among <sec> siblings)."""
    for block in _TABLE_EMITTERS[node.node_type](node, data):
        parent.append(block)


def _emit_titled_table_bundle(parent: etree._Element, node: DocNode, data: dict) -> None:
    """sample-counts-table / bmd-summary / genomics-section → a titled block
    bundle that sits among <sec> siblings, so it gets its OWN <sec> to keep the
    container DTD-valid while holding its document-order slot."""
    sec = E.sec({"id": f"sec-{node.id}"})
    if node.title:
        sec.append(E.title(node.title))
    for block in _TABLE_EMITTERS[node.node_type](node, data):
...
def _emit_handled_by_book_shell(parent: etree._Element, node: DocNode, data: dict) -> None:
    """No body contribution BY DESIGN — the book shell projects these elsewhere:
    cover / title-page → <book-meta>; front-matter → <front-matter> (or the
    abstract into <book-meta>); toc / tables-list → generated by Bookshelf from
    the structure, never authored; page-break → no print concept in XML."""
    return
...
_DISPATCH: dict[str, object] = {
    ...
    "heading-only":        _emit_heading_only,
...
# drifts from the canonical registry.  No allow_omit — every type is decided
# explicitly above, so adding a node type to the registry forces a decision here.
assert_dispatch_covers(_DISPATCH, renderer="BITS/JATS")


def _append_node_content(parent: etree._Element, node: DocNode, data: dict) -> None:
    """Append `node`'s contribution to `parent` (<body> or a <sec>) via the
    per-type `_DISPATCH` table, keeping every container within the JATS
    (blocks)*, sec* content model.  An unregistered type is an error, not a
    silent skip (the registry + tree test guarantee it cannot happen for real
    tree nodes)."""
    handler = _DISPATCH.get(node.node_type)
    if handler is None:
        raise RenderDispatchError(
            f"BITS/JATS renderer has no emitter for node type "
            f"{node.node_type!r} (node id={node.id!r})"
        )
    handler(parent, node, data)
